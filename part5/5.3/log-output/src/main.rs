...
/// HTTP endpoint of the Greeter app (via its Kubernetes Service). The reply is
/// the greeting of whichever version the mesh routed this request to.
const DEFAULT_GREETER_URL: &str = "http://greeter-svc:3000/";
/// Where the ConfigMap's information.txt is mounted (volume).
const DEFAULT_INFO_FILE: &str = "/usr/src/app/config/information.txt";
...
///   file content: <information.txt>           (from ConfigMap volume)
///   env variable: MESSAGE=<value>             (from ConfigMap env)
...
async fn run_reader() {
    ...
    let greeter_url = env::var("GREETER_URL").unwrap_or_else(|_| DEFAULT_GREETER_URL.to_string());
    let info_file = env::var("INFO_FILE").unwrap_or_else(|_| DEFAULT_INFO_FILE.to_string());
    let message = env::var("MESSAGE").unwrap_or_else(|_| String::new());
...
async fn read_logs(
    ...
    // File from the ConfigMap (mounted as a volume).
    let file_content = match tokio::fs::read_to_string(&info_file).await {
        Ok(contents) => contents.trim().to_string(),
        Err(e) => {
            eprintln!("Failed to read info file {}: {}", info_file, e);
            String::new()
        }
    };

    // Env variable from the ConfigMap.
    let env_line = if message.is_empty() {
        "env variable: MESSAGE=<not set>".to_string()
    } else {
        format!("env variable: MESSAGE={}", message)
    };
    ...
    // Fetch the greeting from the Greeter app over HTTP. This is the call the
    // mesh gets to route: the address is one Service, the answer comes from
    // whichever version the HTTPRoute sent it to.
    let greeting = match reqwest::get(&greeter_url).await {
        Ok(resp) => match resp.text().await {
            Ok(text) => text.trim().to_string(),
            Err(_) => "no greeting".to_string(),
        },
        Err(e) => {
            eprintln!("Failed to reach greeter at {}: {}", greeter_url, e);
