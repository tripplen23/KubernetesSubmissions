...
| Folder | Exercise | App | Image | Submit link |
|---|---|---|---|---|
| [`part4/4.1/`](./part4/4.1) | 4.1 — Readiness probe | A rolling update that "succeeds" while the app is broken, then `readinessProbe` on ping-pong and log-output — verified at `0/1` + `1/2`, then `1/1` + `2/2` — and a stalled update rolled back | `europe-north1-docker.pkg.dev/dwk-gke-506208/my-repository/{flaky-app,ping-pong,log-output}:4.1` | [tag `4.1`](https://github.com/tripplen23/KubernetesSubmissions/tree/4.1/part4/4.1) |
| [`part4/4.2/`](./part4/4.2) | 4.2 — The project, step 21: probes and a "break" button | `readinessProbe` on both project Deployments (`todo-backend` proves a fresh Postgres connection), plus a "Break the app" button that takes the pod out of the Service and lets the liveness probe heal it | `europe-north1-docker.pkg.dev/dwk-gke-506208/my-repository/{todo-app,todo-backend}:4.2` | [tag `4.2`](https://github.com/tripplen23/KubernetesSubmissions/tree/4.2/part4/4.2) |
| [`part4/4.3/`](./part4/4.3) | 4.3 — Prometheus | Prometheus, Alertmanager and Grafana through Helm, nine images mirrored first because the cluster cannot reach quay.io, and the exercise's query — `count(kube_pod_info{namespace="monitoring", created_by_kind="StatefulSet"})` → **2** | `europe-north1-docker.pkg.dev/dwk-gke-506208/my-repository/{prometheus,prometheus-operator,prometheus-config-reloader,alertmanager,node-exporter,kube-state-metrics,k8s-sidecar,kube-webhook-certgen,grafana}:91.2.3-images` | [tag `4.3`](https://github.com/tripplen23/KubernetesSubmissions/tree/4.3/part4/4.3) |
| [`part4/4.4/`](./part4/4.4) | 4.4 — Your canary | An Argo Rollouts canary gated by an `AnalysisTemplate` on namespace CPU: the expensive release fails its first measurement and is reverted, the cheap one passes 8/8, and a threshold set too low blocks even the good release | `europe-north1-docker.pkg.dev/dwk-gke-506208/my-repository/{ping-pong:4.4,argo-rollouts:v1.10.0,prometheus:v3.14.0-distroless,prometheus-operator:v0.94.0,prometheus-config-reloader:v0.94.0,node-exporter:v1.12.1-distroless,kube-state-metrics:v2.20.0,kube-webhook-certgen:1.8.8}` | [tag `4.4`](https://github.com/tripplen23/KubernetesSubmissions/tree/4.4/part4/4.4) |
| [`part4/4.5/`](./part4/4.5) | 4.5 — The project, step 22: a Done field, and two ways to update | A `done` field through `PUT /todos/<id>`, then the two update strategies measured: **Recreate** (this app's ReadWriteOnce `image-claim` deadlocks a rolling update) and **BlueGreen**, one `promote` switching everyone to v2 | `europe-north1-docker.pkg.dev/dwk-gke-506208/my-repository/{todo-app,todo-backend}:4.5` | [tag `4.5`](https://github.com/tripplen23/KubernetesSubmissions/tree/4.5/part4/4.5) |
| [`part4/4.6/`](./part4/4.6) | 4.6 — The project, step 23: NATS, a broadcaster and a queue group | One NATS event per change, a **broadcaster** forwarding them to a chat sink, and the queue group that makes six replicas safe — 10 messages → 10 with the group, 60 without it | `europe-north1-docker.pkg.dev/dwk-gke-506208/my-repository/{todo-backend,broadcaster,chat-sink}:4.6` (+ mirrored `nats-server`, `nats-box`, `nats-reloader`, `nats-exporter`) | [tag `4.6`](https://github.com/tripplen23/KubernetesSubmissions/tree/4.6/part4/4.6) |
| [`part4/4.7/`](./part4/4.7) | 4.7 — Baby steps to GitOps | The chapter's pull pipeline with Gitea inside the cluster because it cannot reach `github.com`, a Kustomize base + prod overlay, two `Application`s, and measured self-heal plus a commit rolling the app out | `europe-north1-docker.pkg.dev/dwk-gke-506208/my-repository/{log-output:4.7,gitea:1.24,argocd:v3.5.3,dex:v2.45.1,redis:8.2.3-alpine}` | [tag `4.7`](https://github.com/tripplen23/KubernetesSubmissions/tree/4.7/part4/4.7) |
| [`part4/4.8/`](./part4/4.8) | 4.8 — The project, step 24: the project through GitOps | The whole project as a Kustomize base + `overlays/prod` reconciled by ArgoCD from this repository, after giving the private nodes a Cloud NAT so public git hosts and registries answer | `europe-north1-docker.pkg.dev/dwk-gke-506208/my-repository/{todo-app,todo-backend,broadcaster,chat-sink}:4.8` (+ `postgres:16`, `nats:2.14.6-alpine` straight from Docker Hub) | [tag `4.8`](https://github.com/tripplen23/KubernetesSubmissions/tree/4.8/part4/4.8) |
