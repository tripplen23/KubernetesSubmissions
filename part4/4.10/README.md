    ...
    steps:
      ...
      - name: Carry the release tag into the config repository
        ...
        run: |
          ...
          # Application names, which is this clone: one tag name, pushed by hand in
          # the code repository above, carried across here in the same run
          git -C /tmp/dwk-config tag "$GITHUB_REF_NAME"
          git -C /tmp/dwk-config push origin "$GITHUB_REF_NAME"
```
