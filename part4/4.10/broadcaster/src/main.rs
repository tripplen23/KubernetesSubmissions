...
async fn main() {
    ...
    // 4.9: staging runs this broadcaster in "log only" mode — that environment's
    // broadcaster prints the messages instead of forwarding them to a chat service.
    // One flag, two environments, and no second copy of the program.
    let log_only = env_or("BROADCASTER_LOG_ONLY", "false").eq_ignore_ascii_case("true");
    let destination = if log_only {
        format!("logging only, not forwarding to {chat_url}")
    } else {
        format!("forwarding to {chat_url}")
    };
    ...
    let subscription = if queue_group.is_empty() {
        println!("listening on {subject} (NO queue group: every replica gets every message), {destination}");
        nats.subscribe(subject.clone())
            .await
            .unwrap_or_else(|e| panic!("cannot subscribe to {subject}: {e}"))
    } else {
        println!("listening on {subject} (queue group {queue_group}), {destination}");
    ...
    while let Some(message) = subscription.next().await {
        ...
        // 4.9's staging environment: read the message, print it, send nothing.
        if log_only {
            logged += 1;
            println!("[log] #{logged} {payload}");
            continue;
        }
