...
1. Installing a real monitoring stack with **Helm**, and seeing what the chart
   creates (Prometheus, Alertmanager, Grafana, kube-state-metrics,
...
2. Making that install work on a cluster that **cannot reach quay.io, registry.k8s.io
   or ghcr.io**: mirror every image into your own registry first.
...
4. The metrics the *next* exercise needs: `kube_pod_info` / `kube_pod_owner` for "how
   ...
   which the chapter's canary analysis differences over two minutes (Step 4).

No application source is needed: the exercise is about the monitoring stack and the
metrics of whatever runs there. Its only workload is a four-line BusyBox
pod that keeps crashing (Step 4).
...
**This cluster is small (4 × e2-small) and nearly full.** `kube-prometheus-stack`
wants more memory than is free, so check:
...
memory`, there are two ways out (**neither needed if everything reads `Running`**):
...
The values file targets the 4-node cluster: small requests, a **single** Prometheus
replica. Add a node and set `prometheus.prometheusSpec.replicas: 2` to get the
chapter's setup and its "3 pods" answer (Step 3).

---

## Step 1 — mirror the images

This cluster cannot pull from `quay.io`, `registry.k8s.io` or `ghcr.io`
(general internet egress is blocked), and `kube-prometheus-stack 91.2.3` needs
nine images from those registries. My laptop can reach them, so pull there and
push into Artifact Registry:
...
- **`prometheus-config-reloader` is easy to forget**: it is not in the rendered
  manifests; the *operator* injects it as an init container. Miss it and
  the StatefulSet pods sit in `Init:ImagePullBackOff`.
- `docker push` can end with `unexpected EOF` **after** printing a digest. Usually
  ...
  or different, copy registry-to-registry:
...
images, and gives the stack the modest requests this cluster can schedule.
...
Render the chart first: it shows what will be created and catches a forgotten
image:
...
First look at what the chart installed. The chapter's names come from an older
chart version: with **91.2.3** the Prometheus Service is
`prom-kube-prometheus-stack-prometheus` (listening on **9090**, not 80):
...
Port-forward through the **Service**, as the exercise asks (a pod works too, but a
pod is replaced, the Service is not):

```bash
kubectl port-forward svc/prom-kube-prometheus-stack-prometheus -n monitoring 9090:9090
```

Now open <http://localhost:9090> for Prometheus' GUI: the **Graph** tab is where
you type queries, and `Execute` runs them (*Table* shows the value).

![Prometheus' GUI served on localhost:9090](./assets/image4.png)

### The query: pods created by StatefulSets

The exercise wants the number of pods a StatefulSet created. Start from
`kube_pod_info`, which carries the labels to filter with: `namespace`, `pod`,
...
namespace whose `created_by_kind` is `StatefulSet`" → the Prometheus and
Alertmanager pods, exactly what `kubectl get statefulset -n monitoring` shows.
**Predict it by hand:** add up the StatefulSets' replicas.

> The chapter's output is **3** because its setup ran `replicas: 2` (2 Prometheus +
> 1 Alertmanager); this lab runs one replica, so you get 2. To match, add a node and
> set `replicas: 2`. The *number* is not the point; the query is.
...
In the **GUI** the same answer reads `{}` then `2`, which catches everybody out. It is not empty: `{}` is the **empty label set** of the single series
`count()` returned (`Result series: 1`), because an aggregation drops labels and
keeps a number. Keep them for one row per StatefulSet:
...
Three more queries, the chapter's vocabulary:
...
3000:80` opens it at <http://localhost:3000>, admin password from its secret. Not
needed here, but it is the same data rendered nicely.
...
"how many restarts happened in the last two minutes": at ≥ 2 the analysis fails
and the release is rolled back before it reaches everybody. Which metric answers
that, and how does it behave over time?

This lab brings its own workload, so nothing else has to run: a BusyBox
container that starts, waits 20 seconds, and dies. Kubernetes restarts it, exactly
like a bad release that never stays healthy.
...
Now ask Prometheus for the number, as you would about a release:
...
- It is a counter, so it only goes up. The useful question is "how many restarts
  **since** a moment ago": `offset 2m`, `rate()`, `increase()`.
...
Prometheus is needed by the next exercise, so keep it. When done:
...
- **A stack is a system.** An operator, a database, two exporters, an alert router
  and a dashboard must all fit the cluster before any data exists.
- **Metrics have producers; labels are the handles.** kube-state-metrics reports
  object state, node-exporter the machines; filter on labels, not names.
...
- **Counters are read as differences** (`offset`, `rate()`, `increase()`), the
  reasoning behind the next canary analysis.
- **Requests are a budget; limits are discovered by running.** Near 100 % of
  `Allocated resources` means memory is promised away, and a full cluster can
  deadlock a routine update; Grafana needed ~436 MiB for plugins, so a 384 MiB
  limit restarted forever.
- **A chart is a contract with the registry.** Every image it pulls, including the
  ones its operator injects at runtime, must be reachable inside the cluster.
- **Documentation ages faster than charts.** The chapter's Service name, namespace and
  pod count predate your installed version; your cluster is the specification.