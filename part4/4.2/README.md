...
`todo-backend` + Postgres in the `project` namespace). Nothing else changes; the
other manifests stay put.

What we will learn by *doing*:

1. What a health endpoint must check: a living dependency, not a constant
   answering 200.
2. Readiness vs liveness on a real app: which takes a pod out of the Service,
   which restarts it, and why a *dependency* must never be wired to liveness.
3. How an app that "breaks" itself heals with nobody doing anything: the broken
   flag lives in memory, so the restarted container is healthy.
4. Why a failing readiness probe means the app really **stops answering** on its
   address: a Service routes traffic only to Ready pods, the chapter's "the app
   stops responding to the address after a while".
5. Rolling a bad update back: `kubectl rollout history`, `rollout undo`,
   `--to-revision=N`, the tools the chapter uses after a buggy deploy.
6. Where a **`StartupProbe`** fits (it delays the liveness probe while a slow app
   starts).
...
Everything happens in the `project` namespace, so nothing collides.
`kubectl config set-context --current --namespace=project` saves typing.

---

## Step 1 — the code: two health endpoints, three states

The applications already contain the change. The point of 4.2 is
that a health check must **check something**; these are the endpoints it was built
around:
...
- **The backend caches nothing.** Every check opens a connection and runs the
  query, so a Postgres that goes away or returns is noticed within one probe
  period.
- **Liveness and readiness do not share an endpoint.** The course's example points
  both probes at `/healthz`, which means "the database is down → restart the app
  pods", so a database restart becomes an app crash loop. Here liveness only looks
  at the app itself (`/livez`), and only *readiness* cares about the database. The
  break button behaves the same way, and the story is defensible.
...
> which the pipeline (and kustomize) replace with `main-<sha>` images. Here you
> type the real image reference directly, with no kustomize.
...
Apply them and wait: the pods restart with the new images and probes:
...
can talk to Postgres, the app once it can also reach the backend.

![alt text](./assets/image.png)

Check the endpoint the probes use:
...
At the bottom of the page there is now a **"Break the app"** button. Press it,
then switch to a terminal watching the pods:
...
**What happens, in order** (all of it without you):

1. The page reloads with the red "this instance has been broken" banner.
2. Within ~15 s the readiness probe fails three times → the pod goes `0/1` and
   `todo-app-svc`'s endpoint list becomes **empty**: out of rotation for new
   requests.
3. At ~30 s the liveness probe (`/livez`) has failed three times → kubelet
   **kills the container and starts a new one** in the same pod. The broken flag
   was only in memory, so the new container is healthy.
4. The pod returns to `1/1`, the endpoint comes back, and the page works again.

![alt text](./assets/image4.png)
![alt text](./assets/image5.png)

Watch the counter and restart reason:
...
Measured on a real cluster; times are from the button press:
...
> (`delay=15s`, `period=5s`, `failure=3`) kills the container after about 30
> seconds, too soon for an application that needs a minute to warm up. That is
> what `startupProbe` is for: until it succeeds, the liveness probe does not
> start, so
> `startupProbe: {periodSeconds: 5, failureThreshold: 24}` buys two minutes. This
> app answers within a second, so there is nothing to wait for; the chapter
> mentions the probe for the same reason.

**The app really stops answering.** While the pod is `0/1`, reload the browser
page: the port-forward is still open, but `todo-app-svc` has no endpoints to route
to, so the page no longer loads. The same from inside the cluster:
...
readiness is not decoration, it is how the Service decides where traffic goes. (If
you miss the ~15-second window before the liveness restart, press the button again
once healthy.)

> The exercise says "a new pod will be started up soon". A failing liveness probe
> actually restarts the **container inside the same pod**: the pod name stays,
> `RESTARTS` grows by one. That satisfies the requirement, because the broken
> state lived in the process. (A new *pod* appears only if the pod itself died,
> e.g. the app called `process::exit`.)

---

## Step 4 — a bad update, and rolling it back

The chapter rolls a buggy version back with `kubectl rollout undo`. Do the same:
make a *bad update* pointing the app at a missing backend. An entry in
`env` wins over the ConfigMap value, so it is a one-line change in
...
What you should see, and why it differs from the break button: `/healthz` answers
500 (`todo-backend unreachable`), so the pod never becomes Ready and the rollout
never completes. `/livez` is still 200, so **nothing gets restarted**: a
...
Two takeaways: the application is unreachable while it is not Ready
(readiness gating the Service, not a crash), and the whole bad episode cost
**zero restarts**.

Now the way out, as the chapter shows:
...
`undo` goes back one revision. If that were bad too, name the revision you want:
the chapter's `--to-revision`:
...
> is dropped and the rest renumbered), so `--to-revision=1` right after an undo
> answers
> `error: unable to find specified revision 1 in history`. Always read
> `kubectl rollout history` right before naming a revision.

> `rollout undo` deletes nothing: it creates a *new* revision with the old
> content, so `rollout history` keeps growing. Annotate before you change
> something, and the history stops being a wall of `<none>`:
>
> ```bash
...
> ```
>
> Put the good value back (remove the `TODO_BACKEND_URL` entry) before moving on.

---

## Step 5 — the other half: "connected to a database"

The exercise asks for an endpoint proving the app *is working **and** connected to
a database*. The backend's `/healthz` is that proof, and you can watch it fail:
...
Both pods are `0/1`: the backend cannot run `SELECT 1`, and the app cannot get a
200 from the backend. Note what does **not** happen: `RESTARTS` does
not move, because no probe restarts anything. That is the liveness/readiness split
working; an app whose liveness probe pointed at a database-dependent
endpoint would be in `CrashLoopBackOff` by now.

> Careful about *when* the database disappears. Scaling Postgres down while the
> pods run (what you just did) leaves the backend pod up and merely not Ready, the
> interesting case. If the backend *starts* while Postgres is absent, `init_db`
> retries for about a minute and then exits, and the pod goes into
> `CrashLoopBackOff`: a pre-existing property the probes cannot fix.
