...
- `docker`: you build and push the four images to Artifact Registry; everything else the cluster pulls itself;
- `git`;
...
- Now prove the route from **inside** the cluster, where ArgoCD stands. Any commit hash
  is a pass; a timeout is the only failure:
...
nodes is tight. Look before you install, and free any leftover namespaces if the
course's Step 0 has not:
...
`~`, `^`, or range expressions like `>=1.0.0 <2.0.0`) are only matched against tags, never branches."* That sentence is the exercise:
...
**The tag is what deploys production.** Not the commit it sits on, not CI's push to
`main`, not a human running `kubectl`: a commit to `main` changes staging's revision alone, and `git push origin 4.9.0` changes production's.

Three consequences before the files:

- **the environments are namespaces, not clusters.** Both `Application`s point at the same `https://kubernetes.default.svc`, separated only by `destination.namespace`; the overlay's `namespace:` transformer rewrites every object it renders, so one base serves two scopes;
- **promotion is a tag, so it is reviewable.** Nothing reaches production that is not also on `main`, since the tag points at a commit there;
- **staging is where the untested thing lives.** Same repository, same manifests, earlier revision: the only kind of staging that stays honest.

> This is the pull model 4.8 built: CI writes a line of YAML, ArgoCD inside the
> cluster reads the repository. What 4.9 adds is *which revision each environment may
> follow*, and that revision is the whole mechanism.

---

## Step 2 — the four apps, the images, and the Dockerfiles

Point Docker at the registry once. The images go to Artifact Registry, not Docker Hub; without this the first `docker push` asks for Docker Hub credentials:
...
The repository ArgoCD reads is the one this folder is committed to. There is no second repository to keep in step: the configuration is a directory, `part4/4.9/config/`, and every push to `main` is what ArgoCD sees. The two release files (`application-staging.yaml`, `application-prod.yaml`) sit beside it in `part4/4.9/` as cluster objects, not configuration the path renders.
...
**The cluster has to reach it**, from inside the cluster rather than your laptop:
...
**What you commit, and where.** From the repository root, the configuration lives under
`part4/4.9/config/`. There is no nested `.git` and no second remote: `git init` inside the
config directory is the one mistake that makes ArgoCD clone an empty repository.

```bash
git status                 # the four Dockerfiles you just typed, before the config exists
```

Nothing to create yet: the files you type in Step 5 are committed with the submission, and *that* commit moves staging. ArgoCD watches `refs/heads/main`, so every commit re-syncs staging, not just those touching `part4/4.9/config/`; the path it reads does not change.
...
Download it into `/tmp`, never the repository: it is tens of thousands of lines of CRDs; the next `git add -A` would sweep it in;

**Three traps in those two commands, in order of how often they bite.**

- **`-n argocd` is not optional on the apply.** The manifest's objects carry no `namespace:`, so the command line's namespace is where ArgoCD lands. Omit it and the control plane installs into `default`, where nothing later looks: a silent failure, because the pods do run;
- **`--server-side` matters.** A plain client-side `apply` of a manifest this size trips over the size of the CRD annotations;
- **`kubectl create namespace argocd` before the apply.** Without it the apply fails with *namespaces "argocd" not found* on the first namespaced object and partially succeeds, worse than failing.

Give it a couple of minutes. The pods pull straight from upstream (`quay.io`, `ghcr.io`,
`public.ecr.aws`), which works because of Step 0's NAT. An `ImagePullBackOff` here is slowness, not a wrong registry.
...
Leave the port-forward running in its own terminal: every UI step below depends on it,
and 8080 is now the UI's port, which is why the project's frontend takes 3001 later.

Open <https://localhost:8080>, accept the self-signed certificate, log in as `admin`
with the password above. The first screen is empty, because there is nothing to sync yet.
...
- **the base carries no `namespace:` field anywhere, and there is no Secret in it.** The overlay's `namespace:` transformer owns the namespace (so the overlays' patches need none either), and the Secret (requirement 6) is applied by hand, outside ArgoCD.
...
Postgres: a StatefulSet behind a headless Service, with the data on a volume claim template and `PGDATA` mounted at the *parent* directory, so `initdb` never sees the filesystem's `lost+found`. All three values come from `postgres-secret`:
...
`postgres-secret` does not exist yet, and until it does the Postgres pod does not start. Deliberate; see the end of this step.
...
The Services the applications talk to, and the reason neither overlay may carry a `namePrefix`:
...
shape, the overlays own the tag. Read the table once; the whole overlay mechanism hangs on it:
...
Six broadcasters share the queue group, so one event is delivered once however many replicas the environment asks for. **The base carries six and no `BROADCASTER_LOG_ONLY`**: forwarding is the program's default and the base does not know its environment:
...
Two files plus the kustomization. Staging differs from production in exactly three ways,
all visible here.
...
The status-quo half of requirement 1: `namespace: staging`. Every object the base holds is rewritten into it; no namespace to keep in step by hand.

**Requirement 4, met here.** The broadcaster patch is all of it: one replica instead of six, and the flag the program reads:
...
Read the two overlays side by side and the exercise is visible: the same `resources: [../../base]`, a different `namespace:`, the same four `images:`, and a different list of patches; `cronjob-backup.yaml` is in one `resources:` list and not the other: requirement 5 in a single line.
...
Production's broadcaster is the base's: six replicas, forwarding. **Requirement 4's other half is the absence of a patch**: nothing here mentions `BROADCASTER_LOG_ONLY`, so the program's `false` default stands.
...
Four things to notice, each a design decision rather than a syntax detail:

- **two containers, one `emptyDir`.** `dump` writes the SQL file, `upload` ships it. An `emptyDir` lives as long as the pod, exactly the lifetime this needs: no PVC, no cleanup;
...
- **`concurrencyPolicy: Forbid`.** A dump still running when the next starts would read the database mid-backup; skipping the run is the cheaper mistake;
- **`serviceAccountName: backup-sa`**, a Kubernetes ServiceAccount **not in the repository**, for the same reason as the Secret (same paragraph below).

### Secrets, applied outside ArgoCD

**Requirement 6, met here.** `postgres-secret` and `backup-sa` are prerequisites of the deployed project, not parts of it, and the exercise's *"you may assume that secrets are readily applied outside of the ArgoCD"* permits keeping them out of the repository. Apply both by hand, into both namespaces:
...
- **The identity comes from the cluster, not a file.** The node pool serves the GKE metadata server, so `backup-sa` picks up Google credentials at run time; nothing to create or store.
- **`--member` is scoped to one namespace.** It names `ns/production/sa/backup-sa` and nothing else; skip the binding and the upload dies with `403 ... does not have storage.objects.create access` while the dump succeeds.
- **`objectAdmin`, not `objectCreator`.** `gcloud storage cp` reads the object back as it transfers, so it needs `storage.objects.get` as well as `create`; with `create` alone the upload fails `403` though the dump worked.

A failed CronJob marks its whole Application **Degraded**, which is why the binding matters: the red badge would point at a fault outside the repository.

**Why ArgoCD never deletes them.** Two independent reasons, and either one is enough:

- they are not in the repository, so nothing renders them, and `prune: true` deletes *removed repository objects*, not object types ArgoCD has never seen;
...
The consequence: a Secret that must not live in a public repository is still available to the pods, and the environment stays reproducible because the repository records the *references* (`secretKeyRef: postgres-secret`), not the value.
...
Three objects applied by hand, present in the cluster, in neither Application's inventory: requirement 6, measured rather than asserted.

Without it, the failure is direct: Postgres cannot start without `POSTGRES_PASSWORD`, and the backend cannot start without the credentials it references. `CreateContainerConfigError` and `statefulset.apps/postgres-ss 0/1` are this missing Secret, nothing more.
...
The two outputs differ by exactly one line: production lists `google/cloud-sdk:slim`, because the CronJob's `upload` container rides along inside that overlay. `postgres:16` appears twice there and once in staging, the CronJob's `dump` sitting one level deeper, and indentation marks the depth.

**If `kustomize build` fails, read the file it names.** In a hand-typed lab the usual cause is a filename that does not match a `resources:` entry: *`accumulating resources from 'persistentvolumeclaim.yaml' … no such file or directory`* is a typo (`persistentvolumnclaim.yaml`), not a Kustomize problem. Compare the name in the error with the kustomization's list; overlays most often name a file that does not exist yet, since you type their `patches:` last.

The other one worth knowing: a patch whose `metadata.name` matches no object fails with *no matches for Id …* and names the patch file. A strategic-merge patch targets by `apiVersion` + `kind` + `metadata.name`, nothing else.
...
That push is a release, and it is worth knowing which: staging's `targetRevision` is `refs/heads/main`, so **this commit already satisfies requirement 2**, before either `Application` exists. Production's constraint is not, which is requirement 3.
...
A list of refs (`… HEAD`, `… refs/heads/main`) is the pass, the thing ArgoCD does before it syncs, so it proves the URL and fails the same way ArgoCD would if the repository were private or unreachable.

---

## Step 6 — two Applications: one built in the UI, both kept as files

ArgoCD does nothing alone. An `Application` says *which repository, which path, which revision, which namespace*, and here the revision separates the two environments. Build staging in the UI first: the fastest way to meet the object and see what the fields do.
...
- **Destination**: Cluster URL `https://kubernetes.default.svc` (in-cluster), Namespace `staging`. Under **SYNC OPTIONS** also tick **AUTO-CREATE NAMESPACE**: nothing creates it until ArgoCD does, and the YAML says the same as `CreateNamespace=true`.

Press **CREATE**. The card appears `OutOfSync`, turns `Synced`, and `Progressing` becomes `Healthy`. Everything in `overlays/staging` is now running and you never touched `kubectl`: Postgres, NATS, four apps, a log-only broadcaster.
...
**That is the exercise working, not a failure.** The UI generates the manifests before it creates anything, and no commit satisfies a semver constraint: with no `4.9.x` tag, there is nothing to point at.

The way around it is the file, which the API accepts as it stands. It is written out in
full under *Both Applications, as files* below: write it there, then apply it.
...
The UI wrote the staging Application; in a submission you write both. Delete what is in the cluster so they do not collide, then apply the files:
...
The card list is the first stop. Each card carries **two badges**, answering two questions:

- **Sync Status**: `Synced` means the cluster matches the revision ArgoCD resolved; `OutOfSync` means it does not yet.
- **Health Status**: `Healthy` means the workloads are up; `Progressing` means they are on their way; `Degraded` means something is failing, and the resource tree is where you look.
...
Production's side stays the literal constraint until a tag exists, which breaks nothing,
and Proof 2 changes it. Staging's side is a commit, because a branch is always resolvable.

If both badges are **blank**, nothing is reconciling: `argocd-application-controller` fills them in, so `kubectl -n argocd get pods` comes before any doubt about the Application.

Click a card and you get the **resource tree**: everything the Kustomization rendered: `StatefulSet → Pod` for Postgres and NATS, `Deployment → ReplicaSet → Pod` for the four apps, `CronJob → Job → Pod` in production only, Services beside them. Click a node for its events and live manifest.

**Where the replica count is.** On the **Deployment node**: the `6/6` or `1/1` beside an app's name is `readyReplicas`/`desired`, the healthy-pod count. The broadcaster reads `1/1` in staging, `6/6` in production once a tag puts it there. Two commands read the same numbers:
...
**Empty is the right answer there, not a missing step.** No tag has been pushed, so production has nothing to run; the `6/6` appears in Proof 2, after the tag.

**SYNC, REFRESH and HARD REFRESH**: *Refresh* re-reads the repository without waiting for the 180 s poll; *Sync* reconciles immediately; *Hard Refresh* also drops cached manifests, so press it after editing a file the UI still shows stale. **None changes the repository**; they only make ArgoCD notice it sooner.

**HISTORY AND ROLLBACK**: one entry per revision ArgoCD has deployed.

**DEPLOY** beside an older entry is a rollback, and with `selfHeal` on it lasts until the next sync restores the repository's version; the durable rollback is a revert commit. The **Details** panel names the initiator and the time taken (**automated sync policy**, or a user when started by hand).
...
Each proof measures one of the exercise's requirements, and has a receipt.

### Proof 1 — a commit to `main` deploys staging and leaves production untouched

**Requirement 2**, and the negative half of requirement 3. Change one line in the staging overlay: the difference the overlay exists for:
...
ArgoCD polls every 180 s, so this takes a couple of minutes unless you press **Refresh**. Watch: the `the-project-staging` card turns `OutOfSync`, the todo-app node spins a new ReplicaSet and pod, then settles to `Synced` / `Healthy`, while `the-project-prod` never changes.
...
**What the receipts show.** Staging's `.status.sync.revision` is the commit you just pushed; production's line is still the constraint itself, because only a tag resolves it. Nothing was done to either Application between the two commands: the whole mechanism, and requirement 2 settled.

The page proves it too: the version line under the title reads `staging-2`, and nobody touched the cluster.
...
`5.0.0` matching is not a bug in your constraint, it is what `>=` means, and it is why a promotion tag is a decision and not a formality. Pin it for the other behaviour (`~4.9` stays inside 4.9.x).

**A tag and a branch with the same name.** If you ever tag something `main`, `targetRevision: main` becomes ambiguous; see Step 6 for why `refs/heads/main` is the unambiguous spelling.

### Proof 3 — staging's broadcaster logs instead of forwarding

**Requirement 4**, and the proof that needs two log streams and one counter. In staging the broadcaster says so outright at startup:
...
**The counter is the receipt that forwarding did not happen.** The chat sink's `/count` endpoint says how many messages it has accepted. Run it **after** creating the todo above; with no todo, both counters read `0`.
...
`0` in staging, however many todos you create. Production's counter moves, because its broadcaster is the base's: forwarding, six replicas, no flag. **Same image, same source, one environment variable.**

That is worth a line in your notes: a feature flag is cheaper than a second program, and `BROADCASTER_LOG_ONLY` is why staging can be a *staging*: the notification path runs end to end without telling a real chat service about test todos.
...
The CronJob only fires at 03:15, so the honest way to see it work is to run one by hand
from it: same Job template, same containers, same ServiceAccount:
...
**When a backup fails, the whole Application turns `Degraded`.** A CronJob with a failed Job in its history is unhealthy, and production's `Application` owns it, so the badge turns red.
...
Then delete the manual Job, so the environment matches the repository and the badge is honest:
...
Everything so far was done by hand, twice: build, push, edit the overlays, commit, tag. The chapter's workflow does the same to the same files. It is **optional** here (the tag deploys production, and a by-hand tag is a tag), kept as the submission's record.
...
- **the tag is what deploys production**, exactly as in Step 7. The first run tags its own release commit, and *that* tag satisfies `>=4.9.0`; a commit alone never will;
- **`[skip ci]` is not politeness, it is load-bearing.** The commit CI makes touches `part4/4.9/**`, which triggers the workflow; without the marker the release re-triggers itself. It also stops the tag push below re-running the job, since that commit carries the same message;
- **`contents: write` and `id-token: write` are both required.** The first lets the workflow push; the second is what Workload Identity Federation mints its token from, and without it auth fails with *did not inject `$ACTIONS_ID_TOKEN_REQUEST_TOKEN`*;
- **the workflow must live at the repository root.** GitHub runs workflows only from `.github/workflows` at the repository root: *"You must store workflow files in the `.github/workflows` directory of your repository"*. A copy inside `part4/4.9/.github/workflows/` never runs; the failure is silence.

The shape to notice: **CI never talks to the cluster.** It publishes an image, writes a line of YAML, and pushes a tag. What happens next is ArgoCD's business, in both environments, without a `kubectl`.
...
namespace alone just makes the controller build it again; the `Application`s go first,
both of them.
...
- **the config stays.** `part4/4.9/config/` and the two `application-*.yaml` files are the submission, the deliverable; there is no repository to remove because it is your own;
- **the hand-made objects go with the namespaces.** `postgres-secret` in both and `backup-sa` in production are deleted with `kubectl delete namespace`; nothing to hunt for, *because* they were never ArgoCD's;
...
**The one cleanup that is not a command.** `git tag 4.9.1` and `git tag 5.0.0` from Step 7 stay in the repository, and production's `>=4.9.0` follows whichever is newest next time you point an `Application` at it. To pin production to the release you meant, delete the stray tags *and* pin the constraint:
...
- **A revision is a policy, not a pointer.** `refs/heads/main` for staging and `>=4.9.0` for production: a branch moves with every commit, a semver constraint matches only a tag, which enforces "commit to staging, tag to production" in the tool, not in discipline.
- **One base, two overlays, and the differences are countable.** Read `overlays/staging` beside `overlays/prod`: *what is different in production?* A namespace, a replica count, one environment variable, a CronJob: countable lines of YAML, reviewable in a diff, impossible to forget.
- **The same mechanism scales down to a flag.** Staging's broadcaster is production's with `BROADCASTER_LOG_ONLY: "true"`: same image, same events, same queue group, no external service told about test todos; a second program would be a second thing to keep in step.
- **Not every prerequisite belongs in the repository.** The Secret and the ServiceAccount are applied by hand, and ArgoCD never prunes them: it only deletes what it created. Reproducibility lives in recording the *references* to them, the reviewable part.
- **A tag is a decision with a duration.** `>=4.9.0` will happily deploy `5.0.0` the day someone pushes it; `~4.9` will not. Naming the constraint names how much of your release process you automate.
- **And the tag is the only thing production listens to.** A hundred commits to `main` change staging a hundred times and production not once; one `git push origin 4.9.0` changes production: the whole contract here.
