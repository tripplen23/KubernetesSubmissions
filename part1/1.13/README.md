...
1. `GET /` renders the todo-app HTML page with:
   ...
   - an **input field** (`maxlength="140"`, so the browser refuses more
     than 140 characters),
   - a **Send button** (client-side JS shows an alert, no request is
     sent yet, which is fine per the exercise),
...
> If you did 1.12 the folder already exists, so the command is safe to
...
2. an **input field**: try typing more than 140 characters, and the
...
The project is cumulative, so make sure nothing regressed:
...
1. **Progressive enhancement**: 1.13 only touches the frontend, so the
   image, health and shutdown endpoints from earlier exercises keep
   working.
2. **Client-side constraints**: `maxlength` fits "the input should not
   take todos that are over 140 characters long", enforced by the
   browser before any JS runs.
3. **Placeholder APIs**: `/api/todos` returning hardcoded JSON is a
   contract for the future, and the next exercise will wire the form
   to it (and a database).
