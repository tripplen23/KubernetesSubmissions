...
> **Ingress expects a successful response from a service on the path `/`
> even if the service is mapped to some other path!**

---

## Knowledge: Ingress on GKE (Chapter 4 — "From Service to Ingress")

So far a **Service** was a simple L4 (TCP/UDP) load balancer. **Advanced
traffic rules** (paths, hostnames) need an L7 load balancer: the **Ingress**
resource.

On GKE the Ingress works a bit differently than in k3d/Traefik:

1. **Service type must be `NodePort`** for Ingress in GKE, but GKE **does
   not expose it outside the cluster**: the Ingress controller uses the node
   port only as the backend target. (k3d used ClusterIP + Traefik; here it's
   NodePort + the GKE Ingress controller.)
2. **Health check**: the GKE Ingress health-checks by `GET` on **`/`** and
   expects an **HTTP 200**, so **every** backend must answer `/` with 200,
   **even if it's mapped elsewhere** (e.g. `/pingpong`). The exercise's note:
   ping-pong must reply on `/` (we added a root route) *and* on `/pingpong`.
...
> VMs** (org `opscom.io`), so normal nodes' public IPs fail creation. Use
> **private nodes** (no public IP), policy-safe.
...
Wait for `STATUS: RUNNING` (a few minutes), then **two required fixes**
(needed only if you re-created the cluster):
...
The Ingress needs a few minutes to provision the GCP L7 LB; watch the
ADDRESS:
...
> The Ingress health-checks `/` on each backend; both return 200.
...
1. **Ingress = L7** (paths/hosts); Service = L4 (TCP/UDP).
2. **GKE Ingress needs `NodePort`** services; only the LB uses the port.
3. **Health check on `/`**: every backend must return 200 at root, even if
   mapped elsewhere (why ping-pong got a `/` route).
4. The LB takes minutes to provision; expect brief 404/502.
5. **Delete the cluster when idle**; GKE bills per node/hour + the LB, and
   the Ingress goes with the cluster.
