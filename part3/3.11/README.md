...
`todo-backend` and the cron script are already here; the Dockerfiles and
Kubernetes manifests beside them are the hand-typed part, the DevOps part of
the exercise.
...
Every container gets a `resources` block.
...
requests < limits → `Burstable`; nothing set → `BestEffort`, the first pods
evicted when a node runs out of memory, which is this project today.
...
The nodes are `e2-small` (2 vCPU / 2 GB, so ≈ 940m CPU and ≈ 1.33Gi
*allocatable* each × 4):
...
`kubectl top` shows *usage*, not what the scheduler sees. The third command
shows the **requests already reserved**:
...
**Read that twice: 97 % of this node's memory is already *requested***, and
not by the project:
...
project pod asks for nothing** (`0 (0%)`), the state this lab fixes.

That shapes the numbers below:
...
- All four nodes are already **45–97 % CPU-requested and 66–97 %
  ...
  konnectivity-agent, CSI drivers, kube-state-metrics…). Allocatable ≈
  **5.35Gi**, of which ≈ **4.09Gi** is system-requested, leaving ≈ **1.26Gi**
  for the project.
- `Limits` above 100 % is normal: that column is the overcommit budget, not a
  reservation.
- Request budget per node: `l4lu` ≈ 469Mi, `zca6` ≈ 427Mi, `zfh5` ≈ 355Mi,
...
Total: **250m CPU / 448Mi** requested, fitting the ≈ 1.26Gi headroom the
...
`0 (0%)` in the node table, and `kubectl top` far below the limits; limits are
a ceiling.
...
1. **Memory limit too low → `OOMKilled`.** Measure first:
   `kubectl top pod -n project -l app=todo-backend` → **~0Mi**. This service
   idles near nothing, so an 8Mi limit does **not** kill it; the pod keeps
   running (`RESTARTS 0`). The lesson: pick limits from measurement, not
   guesswork.

   To watch an OOM kill, use a throwaway pod that allocates (verified, exit
   137):
   ```bash
   ...
   container spec, which is why the flag version fails with
   `unknown flag: --requests`.)

   To see the OOM status on the project's own pod, set the limit below its
   real peak; for *this* service there is nothing to see, since it needs
   almost nothing, which is a fine answer for "test what works".
...
2. **Requests too high → `Pending`.**
   ...
   ```
   → scale back with the undo command in point 4.

3. **CPU limits are soft killers:** a CPU-starved container is not restarted
   but *throttled* (latency, not errors). If a service feels slow under load,
   compare `kubectl top pod` with the limit before blaming the code.

4. Undo the experiments (the patch touches only `resources`):
...
- Cluster autoscaling reacts to **requests only**: a pod held back by a big
  request makes GKE add a node.
- This folder deploys its own images (`:3.11`) to `project`, so the deployment
  runs the build from *this* lab, not the older one.

---

## P.S. — what actually happened while doing this lab

- A pod going `Terminating` → `Error` in `kubectl get pods --watch` after a
  change is **the rolling update replacing it**, not a crash: the events show
...
- Setting the backend's memory limit to `8Mi` did **not** cause an OOM kill:
  ...
  stayed `Running` with `RESTARTS 0`: limits come from measurement, and an
  `OOMKilled` demo needs a container that allocates (Step 4).
- `kubectl run` has no `--requests`/`--limits` flags (`unknown flag: --requests`);
  the resources go in `--overrides`.
- The cluster has **no Cloud NAT** (`gcloud compute routers list` is empty), so
  the private nodes have no internet egress. `todo-cron` keeps failing with
  `BackoffLimitExceeded` / `Job has reached the specified backoff limit`,
  because its script fetches `en.wikipedia.org`; this predates the lab and has
  nothing to do with resource limits.
- The repository pipeline still builds from and applies `part3/3.6/...`, so the
  ...
  the next push to `main` would silently strip them. This folder keeps its own
  complete copy, which makes the lab self-contained.
