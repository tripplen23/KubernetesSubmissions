...
This is a **self-contained** lab: it builds on the full 2.8 project
(todo-app, todo-backend, Postgres) and adds the new CronJob. Everything is
included to run from a clean cluster.

## Job vs CronJob (quick recap from the course page)

- **Job** = a workload that runs **once and exits** (it is not a
  server); its status is saved for inspection afterwards.
- **CronJob** = a Job on a **schedule**: every hour it
  launches a fresh Job that does the task and exits.

The CronJob runs a tiny script that:
...
Three images: `todo-app` and `todo-backend` are reused unchanged from
earlier; `todo-cron` is new.
...
> **Location header** of the 302 from `Special:Random` without
...
Run the one-off Job to verify the logic now:
...
Once the one-off Job works, the CronJob fires on the next hour boundary
(`0 * * * *`). Watch it appear:
...
To confirm the schedule without waiting:
...
2. **CronJob** = Job on a schedule (`schedule: "0 * * * *"`), firing a
   fresh Job each hour.
3. **Redirect + Location header**: `curl -w '%{redirect_url}'` reads
   where a 302 points without following it.
4. The CronJob container is **ephemeral**: it runs its one task, exits,
   and a new one starts next interval.
5. **Self-contained lab**: the CronJob depends on the whole 2.8 project,
   so all its source and manifests live here.
